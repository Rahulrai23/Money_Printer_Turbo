...
MoneyPrinterTurbo は単一のテキスト動画生成基盤モデルではなく、動画制作と編集を自動化するパイプラインです。映像素材の取得元は、**動画設定 → 動画ソース** で選択した項目によって決まります。

- **Pexels（デフォルト）、Pixabay、Coverr：** LLM が台本と検索キーワードを作成し、MoneyPrinterTurbo が選択したサービスから一致するストック映像を取得します。これらはサービス上に既に存在する素材であり、プロンプトから新規生成されるものではありません。利用目的に応じて各サービスのライセンスを確認し、遵守してください。
- **ローカルファイル：** アップロードした動画ファイルを編集します。自分が所有しているか、利用許可を得た素材だけを使用してください。本プロジェクトが任意のクリエイターやチャンネルの URL から素材を自動取得することはありません。
- **WaveSpeed AI 動画：** 有料のテキスト動画生成モデル（デフォルトは Seedance）が、台本のキーワードから新しいクリップを生成します。生成前にクリップ単位の課金を確認する必要があります。
- **Shengsuan Cloud AI 動画 / LoomLoom：** 有料の LoomLoom 連携が、見積もりを取得して料金を確認した後に新しい AI 動画クリップを生成します。

どの素材ソースを選んでも、MoneyPrinterTurbo は同じ制作パイプラインでクリップを構成し、ナレーション、字幕、BGM、トランジションを追加して最終動画を書き出します。ストック映像の関連性は自動生成または手入力した検索キーワードに、AI 生成クリップの結果は選択した外部サービスに依存します。
